...
import six
import requests
from social_django import social_core
...
def to_setting_name(*names):
    ...
    aa = '_'.join([name.upper().replace('-', '_') for name in names if name])

    return aa# ''.join('SOCIAL_AUTH_QQ_KEY')
    #return '_'.join([name.upper().replace('-', '_') for name in names if name])


def setting_name(*names):
    # SOCIAL_AUTH_QQ_KEY
    #return 'SOCIAL_AUTH_QQ_KEY'
    #print('AU  ', names)
