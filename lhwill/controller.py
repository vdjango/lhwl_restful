...
def uploadFile(request, config, tid_unix):
    ...
    if config.Base64:
        ...
        # name_move = request.GET.get('filename')
        ...
        try:
            ...
            if 'no' in str(request.GET.get('filename')):
                truelyName = filename
            else:
...
@csrf_exempt
def War_list(request, tid_unix=None):
    connet_list = []
    connets = []
    main = []
    ...
    for line in models.Classification.objects.filter():
        ...
        分类数据
        '''
        k2 = models.Classification_connet.objects.filter(key=line.id)
        for line_1 in k2:
            k3 = models.Classification_connet_connet.objects.filter(key=line_1.id)
            for line_2 in k3:
                connet_list.append(
                    {
                        "n": line_2.title,
    ...
    return HttpResponse(json.dumps(main))


@csrf_exempt
def carouselHander(request, tid_unix):
    UploadFieldName = GetConfigValue("CarouselName")
    AllowExtensions = GetConfigValue("CarouselAllowFiles")
    SizeLimit = GetConfigValue("CarouselMaxSize")
    SavePath = GetConfigValue("CarouselUrl")
    upconfig = UploadConfig('', UploadFieldName,
...
actions = {
    ...
    "alllist": War_list,
    "carousel": carouselHander,
}


def DelDate(tid_unix):
    SUCCESS = "SUCCESS"
    err = []
    path = []
    path.append(GetConfigValue("imageUrlPrefix"))
    path.append(GetConfigValue("videoUrlPrefix"))
    path.append(GetConfigValue("fileUrlPrefix"))
    for delLine in path:
        if '{{tid_unix}}' in delLine:
            p = str(delLine).split('{{tid_unix}}')[0] + str(tid_unix) + '/'
        else:
            p = str(delLine) + str(tid_unix) + '/'
        p = ROOT + p
        if os.path.exists(p):
            try:
                import shutil
    ...
    jsondata = {"state": str(SUCCESS), "url": str(path), "title": str(path), "original": str(path), "error": err}
    return HttpResponse(json.dumps(jsondata))
    pass
