...
from image_thumbs.settings import THUMBS_GENERATE_ANY_SIZE, THUMBS_GENERATE_MISSING_THUMBNAILS, \
    THUMBS_GENERATE_THUMBNAILS
from lhwill.settings import BASE_DIR, MEDIA_ROOT
...
class ImageWithThumbsFieldFile(ImageFieldFile):
    ...
    def _url_for_size(self, size):
        ...
        thumb_file = os.path.join(
            '{}{}'.format(MEDIA_ROOT, (self.THUMB_SUFFIX % (fileBase, size[0], size[1], extension))))
        url = os.path.join('{}{}'.format(MEDIA_ROOT, self.name))

        if THUMBS_GENERATE_MISSING_THUMBNAILS:
            if not self.storage.exists(thumb_file):
                try:
                    self._generate_thumb(self.storage.open(url), size)
                except:
                    if settings.DEBUG:
                        import sys
                        print(sys.exc_info())
        ...
        print('url', url, 'thumb_file', thumb_file)
        print('fileBase', fileBase, 'extension', extension)
    ...
    def _generate_thumb(self, image, size):
        ...
        thumb_name = os.path.join('{}{}'.format(MEDIA_ROOT, (self.THUMB_SUFFIX % (base, size[0], size[1], extension))))
